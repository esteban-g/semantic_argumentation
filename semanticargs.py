...
def well_founded(prg: Program) -> Tuple[List[Symbol], List[Symbol],List[Symbol]]:
    '''
    ...
    # print(f'{" ".join(map(str, interpretation))}')
    ...
    return sorted(fct), sorted(ukn), sorted(falsos)


class LevelApp(Application):
    def __init__(self):
        self.program_name = "Well-founded structured arguments"
        self.version = "1.0"
    ...
    def main(self, ctl: Control, files):
        ...
        fct, ukn, fls = well_founded(prg)
        print('Facts:')
        print(f'{" ".join(map(str, fct))}')
        print('Unknown:')
        print(f'{" ".join(map(str, ukn))}')
        print('False:')
        print(f'{" ".join(map(str, fls))}')
